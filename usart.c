...
bool USART_PutChar(char c){
	if(RingBuffer_PutChar(&USART_RingBuffer_Tx, c))     // if PutChar accepted, enable IRQ_Handler
    {
...
size_t USART_ReadData(char *data, size_t maxSize){
    ...
    while (numberOfChars < maxSize)
    {
        if (USART_GetChar (data + numberOfChars)) {
            numberOfChars++;
        } else {
...
void USARTx_IRQHandler(void){
    ...
    if (__HAL_USART_GET_IT_SOURCE(&UartHandle, USART_IT_TXE)) 
    {
        ...
        char ringBuffer_sign;
        if(RingBuffer_GetChar(&USART_RingBuffer_Tx, &ringBuffer_sign))   
        {            
            USARTx->DR = ringBuffer_sign;
        } else {
